...
def generate_verbose_json(pipeline_result, file_name, tokenizer) -> WhisperVerbose:
    segments = []
    final_text = ""

    for idx, text in enumerate(pipeline_result["segments"]):
        tokens = tokenizer.encode(text["text"], add_special_tokens=False).ids
        segments.append(
            Segment(
                id=len(segments),
                seek=0,
...
class Predictor:
    ...
    def setup(self):
        ...
        compute_type = (
            "bfloat16"  # change to "int8" if low on GPU mem (may reduce accuracy)
        )

        self.pipe = whisperx.load_model(
